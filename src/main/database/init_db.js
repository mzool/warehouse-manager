...
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    ...
    const firstTime = path.join(userDataPath, 'firstTime.json')
    if (!fs.existsSync(firstTime)) {
      // First time opening the app
      console.log('Welcome! This is your first time opening the app.')

      // Create a file to mark the first-time flag
      fs.writeFileSync(firstTime, JSON.stringify({ firstTime: false }))
